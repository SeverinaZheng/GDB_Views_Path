...
non_materialized_dir_path = 'test/view_use/non_materialized'
num_view = 24
...
local_basic_df = pd.read_csv(f'{non_materialized_dir_path}/non_materialized_use_local_basic_{size}.csv')
local_complex_df = pd.read_csv(f'{non_materialized_dir_path}/non_materialized_use_local_complex_{size}.csv')
global_df = pd.read_csv(f'{non_materialized_dir_path}/non_materialized_use_global_{size}.csv')


#################### Local Basic Plot ###############################

local_basic_warm_time =  local_basic_df['Average_Non_Materialized_View_USE_Time'].tolist()
...
plt.ylabel("Average Non Materialized USE Time (ms)")
...
plt.savefig(f"test/evaluation_plots/use_non_materialized/non_materialized_time_local_basic_{size}.pdf", format="pdf", bbox_inches="tight", dpi=150)
...
local_complex_warm_time =  local_complex_df['Average_Non_Materialized_USE_Time'].tolist()
local_complex_cold_time =  local_complex_df['Average_Non_Materialized_Cold_USE_Time'].tolist()
...
#plt.tick_params(axis='y', which='minor')
#ax.yaxis.set_minor_formatter(FormatStrFormatter("%0.1f"))
plt.savefig(f"test/evaluation_plots/use_non_materialized/non_materialized_time_local_comlpex_{size}.pdf", format="pdf", bbox_inches="tight", dpi=150)

plt.close()

#################### Global Plot ###############################

global_warm_time =  local_complex_df['Average_Non_Materialized_USE_Time'].tolist()
#local_complex_cold_time =  local_complex_df['Average_Non_Materialized_Cold_USE_Time'].tolist()
use_query_names = local_complex_df['USE_Query'].tolist()

# The x position of bars
barWidth = 0.3
r1 = np.arange(len(global_warm_time))
r2 = [x + barWidth for x in r1]
 
plt.bar(r1, global_warm_time, width = barWidth, color = 'blue', edgecolor = 'black', capsize=7)
...
plt.xlabel("USE Query")
plt.ylabel("Average Non Materialized USE Time (ms)")
plt.xticks([r + barWidth for r in range(len(global_warm_time))], use_query_names)
plt.xticks(rotation=315)
#plt.legend(bbox_to_anchor = (1.3, 0.9), loc='center right')
#ax = plt.gca()
#ax.set_yscale('log')
...
#plt.tick_params(axis='y', which='minor')
#ax.yaxis.set_minor_formatter(FormatStrFormatter("%0.1f"))
plt.savefig(f"test/evaluation_plots/use_non_materialized/non_materialized_time_global_{size}.pdf", format="pdf", bbox_inches="tight", dpi=150)
