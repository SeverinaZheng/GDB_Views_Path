...
output_filename = f'{dir_path}/outputs/warm_{size}.txt'
output_cold_with_init_laod_filename = f'{dir_path}/outputs/cold_init_load_{size}.txt'
...
with open(output_filename,'r') as data_file:
    for line in data_file:
        if line.startswith('CREATE VIEW AS'):
        ...
        elif 'Return set' in line:
            #print(line)
            view_size = int(line.split()[3])
            isPath = True
        elif 'There are' in line and not isPath: 
            view_size = int(line.split()[2])
        elif 'There are' in line and isPath: 
            view_size += int(line.split()[2])
            ...
            view_size_table[view_index] = view_size
...
print("************")
print(view_create_cold_init_load_time_table)
print("************")'''
...
'''print(avg_create_time)
print("************")
print(avg_cold_create_init_load_time)
print("************")'''

# Writing mean of view_creation and view_cold_creation to a dataframe
df = pd.DataFrame(list(zip(views_arr, avg_create_time, avg_cold_create_init_load_time)), columns =['View_Name', 'Average_View_Creation_Time', 'Average_Cold_Creation_With_Init_Load_Time'])
df.to_csv(f'test/view_creation/view_creation_time_{size}.csv', encoding='utf-8', index=False)
